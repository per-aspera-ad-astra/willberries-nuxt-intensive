      ...
      <table class="cart-table">
        ...
        <tbody class="cart-table__goods">
          <tr class="cart-item" v-for="item in cart" :key="item.id">
            ...
            <td>{{ item.price }}$</td>
            <td>
              <button class="cart-btn-minus" @click="decreaseQuantity(item)">
                -
              </button>
            </td>
            <td>{{ item.count }}</td>
            <td>
              <button class="cart-btn-plus" @click="increaseQuantity(item)">
                +
              </button>
            </td>
            <td>{{ item.price * item.count }}$</td>
            <td>
              <button class="cart-btn-delete" @click="removeFromCart(item)">
                x
              </button>
            </td>
          </tr>
        ...
        <tfoot>
          <tr>
            ...
            <th class="card-table__total" colspan="2">
              {{ getTotalPrice() }}$
            </th>
          </tr>
        </tfoot>
